...
func CreateUserPowerTx(tx *sql.Tx, up *UserPower) error {
	createSql := "INSERT INTO " + TableUserPower + " SET  user_id = ?, power_bet = ?, power_login = ?, power_deposit = ?, power_withdraw = ?"
...
func GetUserPower(uid int) (*UserPower, error) {
	o := xorm.Orm{}
	up := UserPower{}
	querySql, err := o.Table(TableUserPower).Query().Where("user_id = ?", uid).Do(&up)
	if err != nil {
		return &up, err
	}
